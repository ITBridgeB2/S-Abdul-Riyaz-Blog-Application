...
function UserBlogsPage() {
    ...
    const [editFormData, setEditFormData] = useState({ title: "", content: "", catagorie: "", author: "" });
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        axios.put(`http://localhost:5000/blog/${editingBlog}`, editFormData)
            .then(() => {
                setBlogs(prev => prev.map(b => (b.id === editingBlog ? editFormData : b)));
                setEditingBlog(null);
